...
1. **自动检查**：
   - Description 长度和质量（包含触发词、场景说明）
   - SKILL.md 行数（超过 500 行会警告）
   - 文件路径格式（检测 Windows 风格反斜杠）
   - 避免时效性信息（"2025"、"目前"等词汇）
   - 术语一致性检查
...
根据检查报告优化 skill，解决所有错误项和重要警告项后，继续下一步。

**注意**：此步骤对于确保 skill 遵循最佳实践至关重要。虽然可以跳过，但强烈建议执行以提高 skill 质量。
