...
先 `git fetch` 获取远端最新状态，再用 `git status` 看本地是否落后 / 分叉：
...
根据结果处理：
- **`up to date` / 仅领先远端**：直接进入步骤 1。
- **`behind by N commits, can be fast-forwarded`（落后但无本地提交）**：执行 `git pull --ff-only`，再进入步骤 1。
- **`diverged`（本地有提交且远端有新提交）**：默认执行 `git pull --rebase` 把本地提交变基到远端新 HEAD 上；遇到冲突先暂停让用户确认，不擅自 `--force` 或 `reset --hard`。
- **工作区有未暂存修改且需要 pull**：先 `git stash`，pull 后再 `git stash pop`，避免把变更卷入 merge/rebase。

> 原因：跳过此步会导致本地提交堆叠在过时的 HEAD 上，后续 push 会触发非 fast-forward 拒绝，或逼出 merge commit 污染历史。
