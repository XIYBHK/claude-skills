---
name: claude-api Claude 接口
description: "构建、调试和优化 Claude API / Anthropic SDK 应用。使用此 skill 构建的应用应包含 prompt caching。同时处理 Claude API 代码在不同 Claude 模型版本之间的迁移（4.5 → 4.6、4.6 → 4.7、已退休模型替换）。TRIGGER：代码 import `anthropic`/`@anthropic-ai/sdk`；用户询问 Claude API、Anthropic SDK 或 Managed Agents；用户在文件中添加/修改/调优 Claude 功能（caching、thinking、compaction、tool use、batch、files、citations、memory）或模型（Opus/Sonnet/Haiku）；关于 Anthropic SDK 项目中 prompt caching / cache hit rate 的问题。SKIP：文件 import `openai`/其他供应商 SDK、文件名如 `*-openai.py`/`*-generic.py`、供应商中立代码、通用编程/机器学习任务。"
