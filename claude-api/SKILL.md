---
name: claude-api API开发
description: 使用 Claude API 或 Anthropic SDK 构建应用程序。当代码导入 `anthropic`/`@anthropic-ai/sdk`/`claude_agent_sdk`，或用户需要使用 Claude API、Anthropic SDK 或 Agent SDK 时触发。当代码导入 `openai`/其他 AI SDK、通用编程或机器学习/数据科学任务时不触发。
